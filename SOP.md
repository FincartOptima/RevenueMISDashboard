...
## ⚠️ Read this first — current known issue

As of the last check, the GitHub repository `github.com/FincartOptima/RevenueMISDashboard` contains **only `logo.png`** — `app.js`, `data.js`, `index.html`, and `extract.py` are **not present on GitHub**, even though they were uploaded earlier in the project's life. This means the live GitHub Pages link is almost certainly broken or showing stale/incomplete content right now.

**Action required before trusting the live link:** open `github.com/FincartOptima/RevenueMISDashboard` and check the file list. If `app.js`/`data.js`/`index.html` are missing, re-upload all four files from the project folder (see "Monthly Update Procedure" below for the exact steps). Until that's done, the "just upload and trust it" workflow the rest of this SOP describes does **not** apply — you'd be looking at a broken page.

---

## 1. What this dashboard is for

This is a web-based reporting dashboard for Fincart's Revenue MIS (Management Information System). It turns one large, complex Excel workbook (`.xlsb` — Excel Binary format) that the operations team maintains manually into a fast, filterable, always-up-to-date web page that anyone (sales leadership, RMs, finance) can open in a browser — no Excel needed.
...
## 2. What input the engine expects
...
The dashboard reads these sheets **by exact name** and by **fixed column position** (e.g. "column 52 is Revenue" in the INS sheet). If the workbook's sheet names or column order ever change, the code must be updated to match (see Section 9, "Drawbacks").

### 2.2 Row filtering rules baked into the engine
- **B2C only.** Every row in `INS`, `FEES`, `PMS` has a "B Type" column. Only rows marked exactly `B2C` are kept — B2B rows are dropped for those three raw sheets. (Note: the *Dashboard's* top KPIs use `REVENUE REPORT MONTHLY/YTD` instead, which has its own Team column, and there the engine explicitly excludes any row where `Team = B2B` — see Section 5.)
...
### 2.3 What the engine does NOT need from you
- You do **not** need to manually specify the fiscal year, the current month, or the list of RMs/teams — all three are **auto-detected** from the file every time it's processed (see Section 4).
...
### 3.1 Two ways the dashboard gets its data
...
### 3.2 Why Python + Excel COM, not a pure-JS parser?
...
There is also a **secondary, in-browser parser** inside `app.js` (`parseWorkbook()`, using the SheetJS library) that mirrors the Python logic. This exists for the manual "↑ Re-upload .xlsb" button in the header, so someone without Python/Excel installed can still refresh the dashboard by uploading the file straight into the browser. It applies the exact same column mappings and filters as `extract.py` — the two are meant to be kept in sync (see Section 9).

---

## 4. How it computes things — the mechanics

### 4.1 Dynamic fiscal-year detection
...
### 4.4 The MTD "Day Window" filter (INS / FEES / PMS → Monthly Trend)
...
## 5. What it computes — tab by tab

### 📊 Dashboard
- **MTD/YTD toggle** — see 4.3.
- **Top KPI cards:** Total Revenue, Insurance Revenue, FEES Revenue, PMS Revenue, Trail Revenue (MF), RMs Reporting. Sourced from `REVENUE REPORT MONTHLY`/`YTD`, **excluding B2B and pool rows** (see 4.5), and further narrowed if a Team filter is applied.
...
3. **Monthly Trend** — the Day-Window MTD filter (4.4), a monthly chart, and a monthly breakdown table.
...
## 6. Results — and how to manually crosscheck them in Excel
...
### 6.1 Reconciled Dashboard Total (MTD)
...
### 6.2 Reconciled Dashboard Total (YTD)
...
### 6.3 Product-line KPI cards (Insurance / Fees / PMS / Trail)
...
### 6.4 Raw tab totals (INS / FEES / PMS revenue KPIs at the top of each tab)
...
### 6.6 General crosscheck approach for any new chart
1. Click its **info button** — note the sheet name and column numbers.
2. Open that sheet in Excel, and build a quick `SUMIFS`/`COUNTIFS`/pivot table using those exact columns and the "B2C" / FY-month filters mentioned in Section 2.2.
3. Compare to the dashboard number. Small mismatches (a few rupees) can be floating-point rounding; large mismatches mean a column mapping or filter has drifted from the current sheet layout (see 9.1).

---

## 7. Monthly update procedure (operational SOP)

This is what you actually do every month when a new MIS file arrives.
...
2. **Run the extraction script.** Open a terminal/Command Prompt in that folder and run:
   ...
   ```
   If any step errors out or row counts look drastically wrong (e.g. 0 rows), stop and investigate before publishing (see Section 8's troubleshooting notes).
...
4. **Verify.** Open the live GitHub Pages URL, hard-refresh (`Ctrl+Shift+R`), and check the "Generated" timestamp shown under the dashboard title matches your just-run extraction. Spot-check one number against Section 6's crosscheck steps.

You do **not** need to touch any code for a routine monthly update — new months, new RMs, new transaction rows are all picked up automatically (see Section 4 and Section 8.2 for the exceptions).

---

## 8. Known drawbacks and gotchas

### 8.1 Fixed column positions — the biggest fragility
...
### 8.2 Sheet names are hardcoded too
...
### 8.3 Two parallel code paths that must be kept in sync
...
### 8.6 Talk sheet's "PMS" ≠ PMS sheet's "PMS Revenue"
...
### 8.7 PMS "Confirmed" vs gross revenue
...
### 8.8 No automated tests
There is no test suite. Every verification done so far (and documented in Section 6) has been manual, one-off spot-checking after each change. A source-file structure change or a code change could silently break a number with nothing catching it automatically.

### 8.9 Extraction requires a Windows PC with Excel installed
...
### 8.10 GitHub repo currently missing the deployed files
...
4. **Consolidate the two parser implementations** (8.3) — e.g., have `extract.py`'s JSON output double as test fixtures, and write a small script that diffs what the JS parser would produce against what Python produced for the same file, to catch drift automatically.
5. **Add a "last verified against Excel" timestamp/checklist** to the dashboard itself, so viewers know how fresh the manual crosschecking in Section 6 is.
6. **Move the RM-name matching (Section 8.5) to a fuzzy match** (e.g. trim + normalize case + collapse whitespace more aggressively) to reduce spurious "Unassigned" results, or surface a warning in the dashboard when unmatched RMs are found so it's caught immediately rather than discovered later.
7. **Consider a small automated test** that loads a known sample `data.js` and asserts several headline numbers (Section 6 formulas) equal known-good values, to catch regressions when code changes are made.

---

## 10. Code walkthrough

### 10.1 `extract.py` — the data pipeline
...
2. **Helper functions** (lines 13–85):
   ...
   - `fy_months_from_label(mtd)` — parses a string like `"JUN-2026"` into the ordered 12-month FY list. This is the dynamic FY detection described in Section 4.1.
   ...
   - `iso_date(v)` — converts a datetime, an Excel serial number, or a text date into a clean `"YYYY-MM-DD"` string. Used for the `createDate` fields that power the Day-Window filter (4.4).
...
### 10.2 `app.js` — the dashboard logic
...
- **INS/FEES/PMS tabs (`renderINS`/`renderFEES`/`renderPMS`, starting ~line 601/1078/1252):** structurally near-identical to each other (this is intentional — they were built as copies of each other and kept in sync). Shared helpers `assignRevRank()` (fixed ranking, 4.6) and `teamRevChart()` (the Revenue-by-Team chart) are used by all three, to avoid tripling that logic. Each tab's Monthly Trend view (`insMonthlyView`/`feesMonthlyView`/`pmsMonthlyView`) uses the shared `dayWindowBar()` and `applyDayWindow()` functions (lines 743–772) for the Day-Window MTD filter (4.4) — again, one implementation shared by all three tabs rather than three copies.
...
- **`parseWorkbook()` (line ~1714):** the in-browser mirror of `extract.py`, used only when someone uses the "↑ Re-upload .xlsb" button. Reads the same 7 sheets via the SheetJS library (`XLSX.utils.sheet_to_json`), applies the same B2C/FY filters, and returns the same shaped data object that `DATA` is normally set to. **This must be kept in sync with `extract.py` by hand** — see Section 8.3.

- **`loadFromGitHub()` / `boot()` (lines ~1863–1937):** the startup sequence described in Section 3.1. `boot()` runs automatically once the page's DOM is ready (last line of the file: `if(document.readyState!=='loading')boot(); else document.addEventListener('DOMContentLoaded',boot);`).

- **`TABLE_INFO` + `showInfoPopup()` (lines ~1944 to the end):** the data backing every "i" info button in the UI. Each entry is `{title, desc, cols, source}` (and optionally `note` for a flagged methodology caveat, e.g. the PMS/Talk mismatch in 8.6). **When you add or change a chart/table, add or update its `TABLE_INFO` entry too** — this is the dashboard's built-in, always-visible documentation for end users, and it should always describe what the code actually does (several entries were corrected during this project specifically because they'd drifted from the code).

### 10.3 `index.html` — the shell
...
| "How do I update the data?" | Section 7 |
| "A number looks wrong, how do I check it?" | Section 6 |
| "Why is this RM's team blank?" | Section 8.5 |
| "I need to add a new chart" | Section 10.3 (end) |
| "The source file's columns changed" | Section 8.1 — update both `extract.py` and `app.js`'s `parseWorkbook()` |
