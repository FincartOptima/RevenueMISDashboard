...
**Last verified:** 2026-07-06, against source file `MIS FINCART FY- 2026-2027_29 Jun-26 - (4).xlsb`.
...
- **B2B excluded.** Every row in `INS`, `FEES`, `PMS` has a "B Type" column (column B). Rows where B Type is exactly `B2B` are dropped; all other values (including `B2C` and blank) are kept. This ensures rows with missing B Type are included rather than silently lost.
...
- **"Overall Team Target" row captured separately.** In `REVENUE SUMMARY V 2.0`, row 94 (name = "Overall Team Target") contains the company-level revenue target (₹2 Cr MTD), which differs from the sum of individual RM targets (₹1.94 Cr). This row is extracted into `revsum.overallTarget` and used by the Dashboard's Revenue Target KPI when "All Teams" is selected.
...
- **Top KPI cards:**
  - **Total Revenue** — sourced from `REVENUE REPORT MONTHLY`/`YTD`, excluding B2B and pool rows (see 6.5).
  - **Revenue Target** — from the "Overall Team Target" row in `REVENUE SUMMARY V 2.0` when "All Teams" is selected (company-level target, e.g. ₹2 Cr); falls back to the sum of individual RM targets when a specific team is filtered.
  - **% Achievement** — Total Revenue ÷ Revenue Target.
  - **Insurance Revenue, FEES Revenue, PMS Revenue** — sourced from the **raw INS/FEES/PMS sheets** (not the reconciled report), filtered by the MTD month (in MTD mode) or all FY months (in YTD mode), and further narrowed by Team filter. This ensures they match what you see when summing the Revenue column in each sheet directly.
  - **Trail Revenue (MF)** — sourced from `REVENUE REPORT MONTHLY`/`YTD` (trail is not a separate raw sheet).
  - **RMs Reporting** — count of non-B2B, non-pool rows in the Revenue Report.
...
- **Total Revenue Trend chart** (stacked bar) — combined INS + FEES + PMS by month, with each product segment colour-coded. Positioned above the individual product charts.
- **3 individual trend charts** (Insurance / FEES / PMS) — from the raw `INS`/`FEES`/`PMS` sheets, respecting Month/Team filters.
...
**Insurance, FEES, PMS** on the Dashboard now come from the **raw INS/FEES/PMS sheets** (the same source as each tab's own revenue totals), filtered by the current MTD month or full YTD. To crosscheck: sum the Revenue column in the corresponding sheet, filtered to the relevant month(s) and excluding B2B rows (see Section 8.4).

**Trail Revenue** and **Total Revenue** still come from `REVENUE REPORT MONTHLY` / `REVENUE REPORT YTD`:
...
**Note:** Because Insurance/FEES/PMS use raw sheets while Total Revenue uses the reconciled report, they may not sum exactly to Total — the small difference is Trail revenue plus any inter-sheet reconciliation differences.

### 8.4 Raw tab totals (INS / FEES / PMS revenue KPIs at the top of each tab)
These are a straight `SUMIFS` on the raw sheet, **excluding B2B rows** (not filtering for B2C specifically — blank B Type rows are included):
- **INS Revenue** = `SUMIFS(INS!col52, INS!col1(BType), "<>B2B", INS!col48(Month), <in current FY>)`
- **FEES Revenue** = same idea, `SUMIFS(FEES!col33, FEES!col1, "<>B2B")`
- **PMS Revenue** = `SUMIFS(PMS!col33, PMS!col1, "<>B2B")`
...
### 10.4 Dashboard KPI data sources are intentionally mixed
The Dashboard's **Total Revenue** and **Trail Revenue** KPIs use the reconciled `REVENUE REPORT` sheets, while **Insurance/FEES/PMS Revenue** KPIs use the raw `INS`/`FEES`/`PMS` sheets directly (so they match what you see when summing the Revenue column in Excel). This means Insurance + FEES + PMS + Trail may not exactly equal Total Revenue — the difference is small and comes from the two data sources having slightly different aggregation scopes. This is a deliberate trade-off: product KPIs match their source sheets, and Total Revenue matches the reconciled report.
...
2. **Helper functions** (lines 13–85):
   - `ser2month(n)` / `FY_MONTHS` — a dynamically-computed fallback fiscal year (derived from the current date) used only if auto-detection from the REVSUM header fails.
